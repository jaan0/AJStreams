...
import { toggleChatSchema } from '@/lib/validations/chat';
...
export async function POST(req: Request) {
    ...
    const validation = validateRequest(toggleChatSchema, body);

    if (!validation.success) {
        return validation.error;
    }

    const { partyId, isDisabled } = validation.data;
    const channelName = `private-watch-party-${partyId}`;

    try {
        await pusherServer.trigger(channelName, 'chat-toggle', {
            isDisabled,
        });
